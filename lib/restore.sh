...
# emit_plugin_recovery - after a restore, derive the commands needed to bring
# plugins back. Backups keep the plugin *manifests* but not the marketplace
# clones or plugin code, so we read the restored ~/.claude/plugins manifests and
# write ready-to-paste `/plugin marketplace add …` + `/plugin install …` lines
# to a file, and echo a short summary. JSON parsing is a deliberately light
# grep/sed pass (the tool has no jq dependency); it matches Claude Code's format.
emit_plugin_recovery() {
  local km="$HOME/.claude/plugins/known_marketplaces.json"
  local ip="$HOME/.claude/plugins/installed_plugins.json"
  [ -f "$km" ] || return 0   # no restored plugin manifests → nothing to do

  local out_dir out
  out_dir="$(resolve_backup_dir "${OPT_DEST:-}")"
  mkdir -p "$out_dir" 2>/dev/null || out_dir="$HOME"
  out="$out_dir/restore-plugins.txt"

  {
    printf '# Plugin recovery — generated by claude-restore on %s\n' "$(utc_timestamp)"
    printf '# A backup keeps the plugin LIST but not the marketplace clones or\n'
    printf '# plugin code. Paste these into Claude Code, then run /reload-plugins.\n\n'
    # Marketplaces: GitHub "repo" (owner/name) or git "url".
    grep -oE '"repo"[[:space:]]*:[[:space:]]*"[^"]+"' "$km" 2>/dev/null \
      | sed -E 's/.*"([^"]+)"$/\1/' \
      | while IFS= read -r r; do [ -n "$r" ] && printf '/plugin marketplace add %s\n' "$r"; done
    grep -oE '"url"[[:space:]]*:[[:space:]]*"[^"]+"' "$km" 2>/dev/null \
      | sed -E 's/.*"([^"]+)"$/\1/' \
      | while IFS= read -r u; do [ -n "$u" ] && printf '/plugin marketplace add %s\n' "$u"; done
    printf '/reload-plugins\n'
    if [ -f "$ip" ]; then
      printf '\n# If a plugin is still missing after reload, install it explicitly:\n'
      grep -oE '"[A-Za-z0-9_.-]+@[A-Za-z0-9_.-]+"[[:space:]]*:' "$ip" 2>/dev/null \
        | sed -E 's/^"([^"]+)".*/\1/' | sort -u \
        | while IFS= read -r p; do [ -n "$p" ] && printf '/plugin install %s\n' "$p"; done
    fi
  } >"$out" 2>/dev/null || return 0

  log_step "Plugins: restored the list, not the code. Recovery commands written to:"
  log_info "  $out"
  log_info "In Claude Code, paste the marketplace lines then run /reload-plugins:"
  grep '^/plugin marketplace add' "$out" 2>/dev/null | sed 's/^/    /' >&2 || true
  log_info "    /reload-plugins"
  return 0
}
...
do_restore() {
  ...
  if [ "$CCB_JSON" = "1" ]; then
    ...
    [ "$do_home" = "1" ] && emit_plugin_recovery
